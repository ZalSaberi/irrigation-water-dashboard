...
class ImportPreviewDialog(QDialog):
    ...
    def __init__(self, batch: ImportBatch, file_name: str, parent=None):
        ...
        self.setWindowTitle('پیش\u200cنمایش ورود داده')
        self.setMinimumSize(760, 560)
        self.setLayoutDirection(Qt.LayoutDirection.RightToLeft)
        root = QVBoxLayout(self)
        root.setContentsMargins(22, 22, 22, 22)
        root.setSpacing(12)
        title = QLabel('پیش\u200cنمایش ورود داده')
        ...
        sb.addWidget(QLabel(f'کل ردیف\u200cها: {batch.total_rows}'))
        sb.addWidget(QLabel(f'معتبر: {batch.valid_rows}'))
        sb.addWidget(QLabel(f'نامعتبر: {batch.failed_rows}'))
        sb.addStretch(1)
        root.addWidget(summary)
        table = QTableWidget()
        table.setColumnCount(6)
        table.setHorizontalHeaderLabels(['شناسه', 'منبع', 'تاریخ', 'pH', 'EC', 'SAR'])
        preview = batch.samples[:10]
        ...
        if batch.issues:
            issue_text = '\n'.join((f'ردیف {i.row_number}: {i.message}' for i in batch.issues[:8]))
            issues = QLabel(issue_text + ('\n…' if len(batch.issues) > 8 else ''))
            issues.setObjectName('ErrorText')
            issues.setWordWrap(True)
            root.addWidget(issues)
        buttons = QDialogButtonBox(QDialogButtonBox.StandardButton.Cancel | QDialogButtonBox.StandardButton.Ok)
        buttons.button(QDialogButtonBox.StandardButton.Ok).setText('تأیید و ورود')
        buttons.button(QDialogButtonBox.StandardButton.Cancel).setText('انصراف')
        buttons.button(QDialogButtonBox.StandardButton.Ok).setEnabled(batch.valid_rows > 0)
