...
class ImportResultDialog(QDialog):
    ...
    def __init__(self, report: ImportAnalysisReport, parent=None):
        ...
        self.setWindowTitle('نتیجه ورود داده')
        self.setMinimumWidth(520)
        self.setLayoutDirection(Qt.LayoutDirection.RightToLeft)
        root = QVBoxLayout(self)
        root.setContentsMargins(22, 22, 22, 22)
        root.setSpacing(12)
        title = QLabel('ورود داده تکمیل شد')
        title.setObjectName('PageTitle')
        root.addWidget(title)
        card = Card(soft=True)
        b = QVBoxLayout(card)
        b.setContentsMargins(16, 14, 16, 14)
        b.addWidget(QLabel(f'کل ردیف\u200cها: {report.total_rows}'))
        b.addWidget(QLabel(f'تحلیل و ذخیره موفق: {report.successful_rows}'))
        b.addWidget(QLabel(f'ناموفق: {report.failed_rows}'))
        root.addWidget(card)
        messages = []
        ...
        if messages:
            label = QLabel('\n'.join(messages) + ('\n…' if report.failed_rows > len(messages) else ''))
