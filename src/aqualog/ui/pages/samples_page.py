...
class SamplesPage(QWidget):
    ...
    def __init__(self, service: ArchiveQueryService, parent=None):
    ...
    def _build(self):
        ...
        root.setSpacing(12)
        bar = QHBoxLayout()
        bar.setDirection(QHBoxLayout.Direction.RightToLeft)
        bar.setSpacing(8)
        self.search = QLineEdit()
        self.search.setPlaceholderText('جستجو بر اساس شناسه یا منبع…')
        self.search.setMinimumWidth(240)
        self.source = FilterComboBox()
        self.source.addItem('همه منابع', None)
        self.status = FilterComboBox()
        self.status.addItem('همه وضعیت\u200cها', None)
        self.status.addItem('مناسب', 'suitable')
        self.status.addItem('نیازمند احتیاط', 'caution')
        self.status.addItem('نامناسب', 'unsuitable')
        self.use_dates = QCheckBox('بازه تاریخ')
        ...
        for d in (self.date_from, self.date_to):
            ...
            d.setVisible(False)
        clear = QPushButton('پاک کردن فیلترها')
        clear.setProperty('variant', 'ghost')
        clear.clicked.connect(self.reset_filters)
        ...
        head_widget = QWidget()
        head_widget.setLayoutDirection(Qt.LayoutDirection.LeftToRight)
        head = QHBoxLayout(head_widget)
        head.setContentsMargins(0, 0, 0, 0)
        title = QLabel('آرشیو نمونه\u200cها')
        ...
        cb.addWidget(head_widget)
        self.model = ArchiveSamplesTableModel(self)
        ...
        self.table.setItemDelegateForColumn(8, StatusPillDelegate(self.table))
        self.table.doubleClicked.connect(self._double_click)
        cb.addWidget(self.table, 1)
        self.stack.addWidget(card)
        empty = StatePanel('نتیجه\u200cای پیدا نشد', 'نمونه\u200cای با فیلترهای فعلی وجود ندارد.', icon_name='fa5s.search', action_text='پاک کردن فیلترها')
        empty.action_clicked.connect(self.reset_filters)
        self.stack.addWidget(empty)
        self.error = StatePanel('دریافت آرشیو انجام نشد', 'در خواندن داده\u200cها مشکلی رخ داده است.', icon_name='fa5s.exclamation-triangle', action_text='تلاش مجدد')
    ...
    def _toggle_dates(self, on):
        self.date_from.setVisible(on)
        self.date_to.setVisible(on)
        self.refresh()

    def _filters(self):
        return ArchiveFilters(search=self.search.text().strip() or None, source_id=self.source.currentData(), status=self.status.currentData(), date_from=self.date_from.date().toPyDate() if self.use_dates.isChecked() else None, date_to=self.date_to.date().toPyDate() if self.use_dates.isChecked() else None, limit=1000)
    ...
    def refresh(self):
        try:
            ...
            self.source.clear()
            self.source.addItem('همه منابع', None)
            [self.source.addItem(label if label != sid else sid, sid) for sid, label in sources]
            idx = self.source.findData(current)
            self.source.setCurrentIndex(max(0, idx))
            self.source.blockSignals(False)
            data = self.service.list_samples(self._filters())
            self.model.set_rows(data.rows)
            self.count.setText(f'{data.total_count} نمونه')
            self.stack.setCurrentIndex(0 if data.total_count else 1)
        except Exception as exc:
            self.stack.setCurrentWidget(self.error)
            self.toast_requested.emit(f'خطا در آرشیو: {exc}', 'error')
    ...
    def _double_click(self, index):
        ...
        if sid:
            self.sample_detail_requested.emit(sid)
